...
class NotificationUI {
     ...
     * @param {Object} value 
     */
    addTotalResult(value) {
        // console.time();
        const template = NotificationUI.generateResultTemplate(value);
        // console.timeEnd();
        this.newsContainer.insertAdjacentHTML("afterbegin", template);
    }
    clearContainer() {
     ...
     * @param {Object} value 
     */
    static generateResultTemplate(value) {
        return `
        <div class="col s12 m6">
            <div class="card">
                <div class="card-content">
                    <span class="card-title" >Запрос не найден<br>${value.totalResults}</span>
                </div>
            </div>
        </div>
        `;
